...
const highlights = [
  "Independent Xogmall configuration",
  ...
  return (
    <section id="about" className="bg-white py-14 sm:py-16">
      <div className="container-shell grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
        <div className="rounded-3xl bg-gradient-to-br from-navy via-brand-dark to-brand p-7 text-white shadow-[0_24px_70px_rgba(7,28,62,0.18)] sm:p-9">
          <p className="eyebrow !text-blue-200">About Xogmall</p>
          ...
          <div className="mt-7 grid gap-3">
            {highlights.map((highlight) => (
              <div key={highlight} className="flex items-center gap-3 rounded-2xl border border-white/15 bg-white/10 px-4 py-3 text-sm font-semibold">
                <span className="size-2 rounded-full bg-[#ffbf23]" />
                {highlight}
              </div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {moments.map(({ icon: Icon, label, text }) => (
            <div key={label} className="min-h-40 rounded-2xl border border-blue-100 bg-sky-soft p-5">
